...
```bash
sudo sh -c '(echo "#!/usr/bin/env sh" && curl -L https://github.com/globalbioticinteractions/elton/releases/latest/download/elton.jar) > /usr/local/bin/elton && chmod +x /usr/local/bin/elton' && elton version
```
