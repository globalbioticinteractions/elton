        ...
        try {
            ...
            for (JsonNode hit : result) {
                ...
                if (!datasetKey.isMissingNode()) {
                    datasets.add(datasetKey.asText());
                }
            }
        } catch (IOException e) {
            throw new DatasetRegistryException("failed to read as checklistbank metadata", e);
        }
        return datasets
                .stream()
                .map(id -> StringUtils.prependIfMissing(id, "urn:lsid:checklistbank.org:dataset:"))
